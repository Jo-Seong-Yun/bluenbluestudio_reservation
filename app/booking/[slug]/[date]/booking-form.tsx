...
export function BookingForm({
  ...
  return (
    <form action={action} className="mt-8 space-y-6">
      ...
      <fieldset>
        <legend className="mb-2 text-sm font-medium">시간 선택</legend>
        <div className="grid grid-cols-3 gap-2">
          {slots.map((time) => (
            <label key={time}>
              ...
              />
              <span className="border-border bg-surface hover:border-brand peer-checked:border-brand peer-checked:bg-brand peer-checked:text-brand-foreground block cursor-pointer rounded-lg border py-2 text-center text-sm transition-colors">
                {time}
              </span>
            </label>
          ))}
      ...
      {selectedTime ? (
        <>
          ...
          <label className="flex items-start gap-2 text-sm">
            ...
            />
            <span>
              예약 확인을 위해 이름과 연락처를 수집합니다. 촬영일로부터 1년간
              보관 후 삭제하며, 예약 외 다른 목적으로 쓰지 않습니다.
              <br />
