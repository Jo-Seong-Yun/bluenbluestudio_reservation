...
- [ ] 단위 테스트 — 아래는 반드시 포함
  ...
  - `date_overrides`가 `weekly_hours`를 덮어쓰는지
  - `blocks`가 최종 우선인지
  - 당일 예약이 확실히 차단되는지
  - 버퍼로 다음 슬롯이 막히는지
  - KST 변환 (자정 근처, 월말)
- [ ] 특정 날짜의 슬롯을 반환하는 서버 함수

**완료 기준**: UI 없이 테스트만으로 가용 시간이 정확히 나온다.

### Phase 4 — 관리자 ①: 로그인 + 상품 관리 (약 1.5주)

- [ ] Supabase Auth 로그인, `/admin` 보호
- [ ] 상품 목록 — 순서 변경(드래그), 공개/비공개 토글
- [ ] 상품 추가·수정 폼
  - 이름, 소요시간, 가격, 인원, 버퍼
  - **마크다운 에디터로 설명 작성 + 실시간 미리보기**
  - 대표 이미지 / 갤러리 이미지 업로드 (Supabase Storage)
- [ ] 손님에게 보이는 모습 그대로 미리보기

**완료 기준**: 코드를 건드리지 않고 새 촬영 상품을 하나 만들어 공개할 수 있다.

### Phase 5 — 관리자 ②: 스케줄 관리 (약 1.5주)

- [ ] 요일별 기본 운영시간 설정 화면
- [ ] **주간 캘린더 — 1시간 칸 클릭으로 열림/닫힘 토글**
- [ ] 드래그로 여러 칸 한 번에 차단
- [ ] 날짜 단위 휴무 등록 (시험기간 등 여러 날 한 번에)
- [ ] 차단 사유 메모
- [ ] 손님에게 보이는 것과 동일한 가용 시간 미리보기

**완료 기준**: "다음 주 화요일 3시에 일정 생김" → 30초 안에 해당 슬롯을 막을 수 있다.

### Phase 6 — 손님 예약 플로우 (약 2주) ★ MVP 도달

- [ ] 랜딩 — 스튜디오 소개, 사진, 위치, 예약 버튼
- [ ] 상품 목록 / 상세 (마크다운 설명 렌더링)
- [ ] 날짜 선택 캘린더 — 예약 가능한 날만 활성화
- [ ] 시간 선택 — 그날의 1시간 슬롯 표시
- [ ] 정보 입력 + **개인정보 수집·이용 동의 체크박스 (필수)**
- [ ] 예약 완료 — 예약번호, 입금 계좌 안내, 유의사항
- [ ] 예약 조회 / 취소 — 예약번호 + 연락처
- [ ] 동시 예약 충돌 안내 처리
