...
- [x] 상품 상세 (`/booking/[slug]`) — 마크다운 설명 렌더링 + 날짜·시간·
      신청서가 한 화면에서 이어지는 예약 흐름
- [x] 날짜 → 시간 → 신청서 — 한 화면 안에서 단계별로 펼쳐진다. 날짜를
      고르면 그 아래로 시간 칸이 나타나고, 시간까지 고르면 전체가 왼쪽으로
      좁아지며 오른쪽에 신청서가 나타난다(`components/booking-flow.tsx`).
      이 흐름 때문에 처음엔 자바스크립트 없이도 되던 날짜 선택 달력이
      클라이언트 컴포넌트로 바뀌었다 — 페이지 이동으로는 이 전환 자체를
      만들 수 없어서다. 이전의 `/booking/[slug]/[date]` 페이지는 없앴다.
