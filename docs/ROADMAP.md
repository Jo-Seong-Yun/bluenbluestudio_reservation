...
| 알림       | 솔라피(SMS) + Gmail SMTP(이메일)                | Phase 8에서 둘 다 구현. 손님은 SMS, 사장님은 설정한 채널      |
...
- [x] 상품 상세 (`/booking/[slug]`) — 마크다운 설명 렌더링 + 날짜·시간
      선택 화면
- [x] 날짜 → 시간 — 한 화면 안에서 이어진다(`components/booking-flow.tsx`).
      날짜를 고르면 그 아래로 시간 칸이 펼쳐지고, 화면 가운데 하나의
      카드로 크게 자리 잡는다. 이 펼쳐지는 전환 때문에, 처음엔 자바스크립트
      없이도 되던 날짜 선택 달력이 클라이언트 컴포넌트로 바뀌었다 —
      페이지 이동으로는 이 전환 자체를 만들 수 없어서다.
- [x] 신청서 작성 (`/booking/[slug]/[date]/[time]`) — 시간을 고르면 이
      별도 페이지로 이동한다. 처음엔 신청서까지 같은 화면 안에서 옆으로
      나타나게 만들었다가, 움직임이 과하고 화면이 복잡하다는 피드백을
      받고 별도 페이지로 도로 분리했다. 진입 시 그 시간이 여전히
      예약 가능한지 한 번 더 확인한다(`loadAvailableSlots`).
...
- [ ] 예약 목록 — 상태·기간 필터 — 월간 캘린더로 날짜별 확인은 되지만,
      상태만으로 가로질러 보는 목록/필터는 아직 없다. 캘린더로 대부분의
      운영은 되니 우선순위를 낮췄다. 필요해지면 추가한다.
- [x] 주간/월간 캘린더에 예약 표시 (`/admin/reservations`)
- [x] 확정 / 취소 / 완료 / 노쇼 처리
- [x] 수기 예약 등록 (전화·DM으로 들어온 건) — "수기 예약 등록" 버튼.
      손님용 신청과 같은 계산(loadAvailableSlots)으로 다시 확인하고,
      바로 확정(confirmed) 상태로 넣는다.
- [x] 사장님 메모
- [x] 설정 화면 (`/admin/settings` — 리드타임, 예약가능기간, 취소기한,
      계좌, 공지, 스튜디오 소개)
...
- [x] 사장님: 새 예약 신청 (즉시) — `settings.admin_notify_phone`/
      `admin_notify_email`에 채워진 채널로만 SMS·이메일(Gmail SMTP) 발송.
